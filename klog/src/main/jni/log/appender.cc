...
static klogmapfile sg_mmmap_file;

//标识是否是加密
static bool isCryptFlag = true;
...
static uint64_t file_size(std::string logfilepath) {
    struct stat statbuf;
    stat(logfilepath.c_str(), &statbuf);
...
//    if (boost::filesystem::exists(logfilepath)) {
    if (!access(logfilepath.c_str(), F_OK)) {
//        filesize = boost::filesystem::file_size(logfilepath);
          filesize = file_size(logfilepath);
    } else {
        return true;
    }
...
//        if (!boost::filesystem::exists(tmplogfilepath)) {
        if (access(tmplogfilepath.c_str(), F_OK)) {
...
//        if (!sg_mmmap_file.operator !()) memset(sg_mmmap_file.data(), 0, kBufferBlockLength);
        if (!sg_mmmap_file.error()) memset(sg_mmmap_file.data(), 0, kBufferBlockLength);
