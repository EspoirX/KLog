...
private:
    uint16_t seq_; //同步：0，  异步：每次flush的时候seq自增
    uint32_t tea_key_[4]; //server.public_key && client.private_key配合生成的share shared secrete
    char client_pubkey_[64]; //保存到header中，解密的时候拿这个public_key再和server的private key配合进行解密
    bool is_crypt_;
