...
#define  xlogger_Write_Info(...)		((void)0)
#define  xlogger_Write(...)				((void)0)
#endif

#ifndef LONGTHREADID2INT
#define LONGTHREADID2INT(a) ((a >> 32)^((a & 0xFFFF)))
#endif // LONGTHREADID2INT
...
public:
	...
	~XLogger() {
		...
		if (m_isassert)
			...
			xlogger_Write_Info(m_isinfonull?NULL:&m_info, m_message.c_str());
	}
...
public:
	...
	:m_enable(xlogger_IsEnabledFor(_level)), m_info(), m_tv() {
		...
		if (m_enable) {
			...
			xlogger_Write_Info(&m_info, strout);
		}
	}
...
#define xgroup2_define(group)	   XLogger group(kLevelVerbose, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, XLOGGER_HOOK)
#define xgroup2(...)			   XLogger(kLevelVerbose, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, XLOGGER_HOOK)(__VA_ARGS__)
#define xgroup2_if(exp, ...)	   if ((!(exp))); else XLogger(kLevelVerbose, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, XLOGGER_HOOK)(__VA_ARGS__)
