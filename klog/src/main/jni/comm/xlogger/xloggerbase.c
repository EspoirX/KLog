...
TLogLevel   xlogger_Level() {
    if (NULL == &__xlogger_Level_impl)  return kLevelVerbose;
...
void xlogger_Write_Info(const XLoggerInfo* _info, const char* _log) {
	if (NULL != &__xlogger_Write_impl)
		__xlogger_Write_impl(_info, _log);
}

void xlogger_Write(TLogLevel level, int line, int pid, intmax_t tid, intmax_t maintid, const char* tag,
        const char* filename, const char* funcname, const char* logmsg) {
    XLoggerInfo xlog_info;
    gettimeofday(&xlog_info.timeval, NULL);
    xlog_info.level = level;
    xlog_info.line = line;
    xlog_info.tid = tid;
    xlog_info.pid = pid;
    xlog_info.maintid = maintid;
    xlog_info.tag = NULL == tag ? "" : tag;
    xlog_info.filename = NULL == filename ? "" : filename;
    xlog_info.func_name = NULL == funcname ? "" : funcname;

    xlogger_Write_Info(&xlog_info, NULL == logmsg ? "NULL == log" : logmsg);
}
...
static TLogLevel gs_level = kLevelVerbose;
